...
var _Sources = (() => {
  ...
  var TachiBackRequestInterceptor = class {
    ...
    async interceptRequest(request) {
      const auth = await this.getAuthorizationString();
      request.headers = {
        ...request.headers,
            ...
            rows: async () => retrieveStateData(stateManager).then((values) => [
              App.createDUISwitch({
                ...
                value: App.createDUIBinding({
                  ...
                  async set(value) {
                    values.showContinueReading = value;
                    await setStateData(stateManager, interceptor, values);
                  }
                })
              }),
              ...
              App.createDUISwitch({
                ...
                value: App.createDUIBinding({
                  async get() {
                    return values.showRecentlyAdded;
                  },
                  async set(value) {
                    values.showRecentlyAdded = value;
                    await setStateData(stateManager, interceptor, values);
                  }
                })
              }),
              App.createDUISwitch({
                id: "showLibraryCategories",
                label: "Show Library Categories",
                value: App.createDUIBinding({
                  async get() {
                    return values.showLibraryCategories;
                  },
                  async set(value) {
                    values.showLibraryCategories = value;
                    await setStateData(stateManager, interceptor, values);
                  }
                })
              })
  ...
  async function searchRequest(searchQuery, metadata, requestManager, interceptor, stateManager, cacheManager) {
    ...
    if (result === void 0) {
      ...
      if (typeof searchQuery.title === "string" && searchQuery.title !== "") {
        const SEARCH_QUERY = `
				query searchManga($query: String!, $offset: Int!, $limit: Int!) {
					mangas(
			...
			`;
        try {
          const data = await executeGraphQL(SEARCH_QUERY, {
    ...
    return App.createPagedResults({
      ...
      metadata: paginatedResult.length === pageSize ? { offset: offset + pageSize } : void 0
    });
  }
  ...
  var TachiBack = class {
    ...
    async getMangaDetails(mangaId) {
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...await getMangaDetails(mangaId, this.requestManager, this.stateManager)
        })
      });
    }
    async getChapters(mangaId) {
      ...
      return chapters.map((chapter, index) => {
        ...
        const scanlatorInfo = chapter.scanlator ? ` \xB7 ${chapter.scanlator}` : "";
        return App.createChapter({
          id: `${chapter.id}`,
          chapNum: chapter.chapterNumber || 0,
          name: chapter.name || `Chapter ${chapter.chapterNumber}`,
          time: chapter.uploadDate ? new Date(chapter.uploadDate * 1e3) : /* @__PURE__ */ new Date(),
          group: `${chapter.pageCount} pages ${progress}${scanlatorInfo}`,
      ...
      for (const section of sections) {
        ...
        switch (true) {
          case section.id === "continuereading":
            ...
            break;
          case section.id === "recentlyupdated":
            query = `
          ...
          case section.id.startsWith("category-"):
            ...
            query = `
					...
					`;
            variables = { categoryId, limit: pageSize };
            break;
          default:
            continue;
        }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (true) {
        case homepageSectionId === "continuereading":
          ...
          break;
        case homepageSectionId === "recentlyupdated":
          query = `
          ...
          variables = { offset, limit: pageSize };
          break;
        case homepageSectionId === "recentlyadded":
          query = `
        ...
        case homepageSectionId.startsWith("category-"):
          ...
          query = `
				...
				`;
          variables = { categoryId, offset, limit: pageSize };
          break;
        default:
          return App.createPagedResults({
            results: [],
      ...
      try {
        const data = await executeGraphQL(query, variables, this.requestManager, this.stateManager);
        const tiles = [];
        let items = [];
    ...
    async getMangaProgress(mangaId) {
      const CHAPTERS_QUERY = `
			query getChaptersProgress($mangaId: Int!) {
				chapters(condition: {mangaId: $mangaId, isRead: true}, orderBy: LAST_READ_AT_DESC) {
					nodes {
						id
						chapterNumber
						lastReadAt
      ...
      try {
        ...
        const lastReadChapter = chapters[0];
        return App.createMangaProgress({
          mangaId,
          lastReadChapterNumber: lastReadChapter.chapterNumber || 0,
          lastReadVolumeNumber: 0,
          trackedListName: "Library",
          lastReadTime: lastReadChapter.lastReadAt ? new Date(lastReadChapter.lastReadAt * 1e3) : /* @__PURE__ */ new Date()
        });
      } catch (error) {
            ...
            App.createDUISection({
              ...
              rows: async () => [
                ...
                App.createDUILabel({
                  id: "progress",
                  label: "Progress",
                  value: `${manga.chapterCount - manga.unreadCount} / ${manga.chapterCount} chapters read`
                }),
                App.createDUISwitch({
                  id: "inLibrary",
                  label: "In Library",
                  value: manga.inLibrary || false
                })
              ]
            })
          ];
