...
**Core Architecture**: Single-file bundled source extension (`Kavya/source.js`) that communicates with Tachidesk GraphQL API.

## Key Components

### 1. Main Extension Class (`TachiBack`)
Located at the end of [Kavya/source.js](Kavya/source.js#L1183-1754), this implements the Paperback `Source` interface with:
...
All server interactions use GraphQL queries executed through `executeGraphQL()` helper ([source.js#L783-795](Kavya/source.js#L783-795)):
```javascript
...
Default values defined in `DEFAULT_VALUES` object ([source.js#L851-859](Kavya/source.js#L851-859)). Access via `getTachiBackAPI()` and `getOptions()` helpers.
...
See `serverSettingsMenu()` implementation ([source.js#L886-1024](Kavya/source.js#L886-1024)) for complete example.

## Development Workflow

### Building & Testing
This is a **pre-compiled** extension repository:
1. [Kavya/source.js](Kavya/source.js) is the bundled output (DO NOT edit directly)
...
**To update the extension**: Modify source TypeScript files in the original development environment, rebuild, then replace [Kavya/source.js](Kavya/source.js).

### Version Management
When updating:
1. Increment version in `TachiBackInfo.version` ([source.js#L1167](Kavya/source.js#L1167))
...
Special format in chapter group field ([source.js#L1238-1240](Kavya/source.js#L1238-1240)):
```
